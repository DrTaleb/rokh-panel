...
export default function SliderId({data}) {
    ...
    const submitHandler = async (event) => {
        ...
        if (titleError || textError) {
            ...
            try {
                const res = await axios.put(`${process.env.LOCAL_URL}/api/admin/expertise/edit/${router.query.expertiseId}`,formData,{headers : {
                            'Content-Type': 'multipart/form-data',
                        }
                    }
                )
                Nprogress.done()
                if (res.data.message === "expertise changed"){
                    Nprogress.done()
                    await Swal.fire({
                        icon: 'success',
                        text: "تخصص آپدیت شد",

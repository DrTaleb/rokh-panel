...
export default function AddSlider({data}) {
    ...
    const [expertiseList, setExpertiseList] = useState(data)
    ...
    let exList = []
    useEffect(()=>{
        expertise.length && expertise.map(item => exList.push({label : item.title,value : item.id}))
        console.log(exList)
    },[expertise])
    ...
    const submitHandler = async (event) => {
        ...
        if (titleError ) {
            ...
            try {
                const res = await axios.post(`${process.env.LOCAL_URL}/api/admin/example/add-example`,formData,{headers : {
...
export async function getServerSideProps(context){
    ...
    const data = await datas.expertise
    if (!data){
        return{
            notFound : true
        }
